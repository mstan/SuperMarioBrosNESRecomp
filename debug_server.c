/*
 * debug_server.c — TCP debug server for NES recomp (Faxanadu)
 *
...
static void handle_get_frame(int id, const char *json)
{
    ...
    send_fmt("{\"id\":%d,\"ok\":true,"
             ...
             "\"game_mode\":\"0x%02X\",\"room_id\":\"0x%02X\","
             "\"last_func\":\"%s\"}",
...
static void handle_frame_range(int id, const char *json)
{
    ...
    for (int f = start; f <= end; f++) {
        ...
        pos += snprintf(buf + pos, 256,
            "{\"frame\":%u,\"verify\":%d,\"mode\":\"0x%02X\","
            "\"room\":\"0x%02X\",\"bank\":%d,\"btn\":\"0x%02X\"}",
            r->frame_number, r->verify_pass, r->game_mode,
            r->room_id, r->current_bank, r->controller_buttons);
    }
...
/* ---- Game-specific: Faxanadu ---- */

static void handle_faxanadu_state(int id, const char *json)
{
    (void)json;
    send_fmt("{\"id\":%d,\"ok\":true,"
...
static void handle_entity_table(int id, const char *json)
{
    (void)json;
...
static const CmdEntry s_commands[] = {
    ...
    { "faxanadu_state", handle_faxanadu_state },
    { "entity_table",   handle_entity_table },
...
void debug_server_record_frame(void)
{
    ...
    /* Game-specific (Faxanadu) */
    r->game_mode    = g_ram[0x12];
    r->room_id      = g_ram[0xEB];
    r->player_hp    = g_ram[0x0394];
    r->player_gold_lo = g_ram[0x035C];
